...
function Navbar({navPath}) {
  ...
  return (
    <nav className='flex sm:justify-center space-x-2 mt-4'>
    {Navdata.map(([title, url, bg]) => (
      <Link
        ...
        className={`px-4 text-sm xl:text-base font-medium ${styles['tabtran-'+bg] +' '+ styles['tran']}    py-2 ${styles[bg]} ${styles[bg+':hover']} text-${bg} font-medium rounded-md bg-clip-padding backdrop-filter backdrop-blur-sm border hover:text-white ${
